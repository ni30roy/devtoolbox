...
import { useJsonToYamlWorker } from '@/hooks/useJsonToYamlWorker'
...
const SAMPLE_JSON =
  '{"name":"CodeTool","free":true,"rating":null,"release":{"version":"1.0","year":2026},"tools":["formatter","validator","minifier","beautifier"],"users":[{"name":"A","active":true},{"name":"B","active":false}]}'

export function JsonToYamlWorkbench() {
  ...
  const { run, busy } = useJsonToYamlWorker()
  ...
  const handleDownload = useCallback(() => {
    if (!result?.ok) return
    const base = fileName ? fileName.replace(/\.json$/i, '') : 'json-to-yaml'
    const blob = new Blob([result.output], { type: 'application/x-yaml' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `${base}.yaml`
    link.click()
          ...
          >
            {!result && (
              <p className="pt-8 text-center text-sm text-slate-400 dark:text-slate-600">
                Run Convert to YAML to see the result here.
              </p>
            )}
