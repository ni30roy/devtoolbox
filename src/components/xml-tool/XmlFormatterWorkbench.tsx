...
import { useXmlFormatterWorker } from '@/hooks/useXmlFormatterWorker'
...
export function XmlFormatterWorkbench() {
  ...
  const { run, busy } = useXmlFormatterWorker()
  ...
  const handleDownload = useCallback(() => {
    if (!result?.ok) return
    const base = fileName ? fileName.replace(/\.xml$/i, '') : 'formatted'
    const blob = new Blob([result.output], { type: 'application/xml' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `${base}.xml`
    link.click()
      ...
      <div className="grid divide-y divide-slate-200 lg:grid-cols-2 lg:divide-x lg:divide-y-0 dark:divide-slate-800">
        ...
        >
          ...
          <textarea
            ...
            placeholder={'Paste XML here, drop a .xml file, or click Upload…\n\n<root><item>value</item></root>'}
          ...
          >
            {!result && (
              <p className="pt-8 text-center text-sm text-slate-400 dark:text-slate-600">
                Run Format XML to see the result here.
              </p>
            )}
            {result && !result.ok && <JsonErrorPanel error={result.error} heading="Can't format this XML" />}
            {result?.ok && (
