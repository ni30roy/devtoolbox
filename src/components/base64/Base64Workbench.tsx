...
import { decodeFromBase64, encodeToBase64, type Base64Result } from '@/lib/base64/base64Engine'
...
const SAMPLE_TEXT = 'Hello from CodeTool 🚀 — नमस्ते, こんにちは, 你好, café'
const sampleEncoded = encodeToBase64(SAMPLE_TEXT)
const SAMPLE_BASE64 = sampleEncoded.ok ? sampleEncoded.output : ''
...
export function Base64Workbench() {
  ...
  // Base64 encode/decode is a single linear pass over the input — even a
  // multi-megabyte string finishes in a few milliseconds, so this runs
  // synchronously on the main thread rather than in a Web Worker (unlike
  // the JSON tools' recursive parsing, there's no realistic input size here
  // that would freeze the UI).
  const runAction = useCallback(
    (action: Mode) => {
      setMode(action)
      setCopied(false)
      setResult(action === 'encode' ? encodeToBase64(input) : decodeFromBase64(input))
    },
    [input],
  )
  ...
  const outputLabel = mode === 'encode' ? 'Base64 output' : 'Text output'
  const errorHeading = mode === 'encode' ? 'Could not encode' : 'Invalid Base64'

  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      {/* Action toolbar */}
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 p-3 dark:border-slate-800">
        <div className="flex flex-wrap gap-2" role="group" aria-label="Base64 actions">
          <button
            type="button"
