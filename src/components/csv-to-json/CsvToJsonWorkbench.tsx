...
import { useCsvToJsonWorker } from '@/hooks/useCsvToJsonWorker'
...
import { CopyIcon, DownloadIcon, TableIcon, TrashIcon, UploadIcon } from '@/components/icons/Icons'

const LARGE_INPUT_BYTES = 2 * 1024 * 1024 // 2 MB — above this we show a "this may take a moment" hint

const SAMPLE_CSV = 'name,role,active,city\nAda Lovelace,Engineer,true,London\nAlan Turing,Researcher,false,Manchester\n'

export function CsvToJsonWorkbench() {
  ...
  const { run, busy } = useCsvToJsonWorker()
  ...
  const handleDownload = useCallback(() => {
    if (!result?.ok) return
    const base = fileName ? fileName.replace(/\.csv$/i, '') : 'csv-to-json'
        ...
        >
          <div className="flex items-center justify-between gap-2 px-4 py-2.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                <TableIcon className="h-3.5 w-3.5" /> Sample
              </button>
              <button
                type="button"
          ...
          <textarea
            ...
            placeholder={'Paste CSV here, drop a .csv file, or click Upload…\n\nname,role\nAda,Engineer'}
          ...
          >
            {!result && (
              <p className="pt-8 text-center text-sm text-slate-400 dark:text-slate-600">
                Run Convert to JSON to see the result here.
              </p>
            )}
            {result && !result.ok && <JsonErrorPanel error={result.error} heading="Can't convert this CSV" />}
            {result?.ok && (
