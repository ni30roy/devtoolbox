...
import { decodeFromBinary, encodeToBinary, type BinaryResult } from '@/lib/binary/binaryEngine'
...
const SAMPLE_TEXT = 'Hi 世!'
const sampleEncoded = encodeToBinary(SAMPLE_TEXT)
const SAMPLE_BINARY = sampleEncoded.ok ? sampleEncoded.output : ''

export function BinaryWorkbench() {
  ...
  const runAction = useCallback(
    (action: Mode) => {
      setMode(action)
      setCopied(false)
      setResult(action === 'encode' ? encodeToBinary(input) : decodeFromBinary(input))
    },
    [input],
  )
  ...
  const inputLabel = mode === 'encode' ? 'Text input' : 'Binary input'
  const outputLabel = mode === 'encode' ? 'Binary output' : 'Text output'
  const errorHeading = mode === 'encode' ? 'Could not encode' : 'Invalid binary'

  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      {/* Action toolbar */}
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 p-3 dark:border-slate-800">
        <div className="flex flex-wrap gap-2" role="group" aria-label="Binary actions">
          <button
            type="button"
