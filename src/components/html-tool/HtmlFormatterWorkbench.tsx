...
import { useHtmlFormatterWorker } from '@/hooks/useHtmlFormatterWorker'
...
const SAMPLE_HTML =
  '<!DOCTYPE html><html><head><title>Sample</title></head><body><div class="card"><h1>Hello</h1><p>Welcome to <b>CodeTool</b>.</p><ul><li>One</li><li>Two</li></ul></div></body></html>'

export function HtmlFormatterWorkbench() {
  const [input, setInput] = useState('')
  const [indent, setIndent] = useState<IndentOption>('2')
  const [result, setResult] = useState<HtmlFormatResult | null>(null)
  const [fileName, setFileName] = useState<string | null>(null)
  const [isDragging, setIsDragging] = useState(false)
  const [copied, setCopied] = useState(false)

  const { run, busy } = useHtmlFormatterWorker()
  const fileInputRef = useRef<HTMLInputElement>(null)

  const runFormat = useCallback(async () => {
    setCopied(false)
    const outcome = await run(input, indent)
    setResult(outcome)
  }, [input, indent, run])
  ...
  const handleDownload = useCallback(() => {
    if (!result?.ok) return
    const base = fileName ? fileName.replace(/\.html?$/i, '') : 'formatted'
      ...
      <div className="grid divide-y divide-slate-200 lg:grid-cols-2 lg:divide-x lg:divide-y-0 dark:divide-slate-800">
        ...
        >
          ...
          <textarea
            ...
            placeholder={'Paste HTML here, drop a .html file, or click Upload…\n\n<div><p>Hello</p></div>'}
          ...
          >
            {!result && (
              <p className="pt-8 text-center text-sm text-slate-400 dark:text-slate-600">
                Run Format HTML to see the result here.
              </p>
            )}
            {result && !result.ok && <JsonErrorPanel error={result.error} heading="Can't format this HTML" />}
            {result?.ok && (
